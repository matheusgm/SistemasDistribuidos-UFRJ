...
void runProgram(string path, string id, int r, int k) {
    ...
    string run = "./" + file_without_extension + id + ".out " + id + " " + to_string(r) + " " + to_string(k);
...
int main(int argc, char const *argv[]) {
    ...
    thread thread_coordinator(runProgram, program1Path, "", r, k);
    vector<thread> thread_process(n);
    for (int i=0; i<n; i++) {
        thread_process[i] = thread(runProgram, program2Path, to_string(i), r, k);
    }
